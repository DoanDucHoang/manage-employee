...
export function handleOriginRoutes(req: IncomingMessage, res: ServerResponse): boolean {
    ...
    if (req.method === 'POST' && pathname === "/api/origins") {
        originController.createOrigin(req, res);
        return true;
    }

    const match = pathname.match(/^\/api\/origins\/([0-9]+)$/);
    if (match) {
